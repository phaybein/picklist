...
it('passes browser cookies source to metadata fetching when configured', function () {
    $home = testHome();
    $binary = fakeBinary($home.'/bin/yt-dlp');
    $config = new AppConfig(
        playlistId: 'PLabc123',
    ...
    app()->instance(VideoMetadataFetcher::class, new class($calls) implements VideoMetadataFetcher
    {
        public function __construct(private array &$calls) {}

        public function fetch(string $url, string $binaryPath, string $cookiesFromBrowser = ''): array
        {
            $this->calls[] = compact('url', 'binaryPath', 'cookiesFromBrowser');
